...
interface LoanAccountData {
  ...
  currency: string;
  status: 'communications' | 'occurrences';
}
...
export const OccurrencesSection: React.FC<PassivesSectionProps> = ({
    ...
    <div className="grid gap-2 content-start">
      ...
      >
        {loanAccounts.map((account) => {
          const itemData: ItemData = {
            id: createUniqueId('calls', 'communications', account.id),
      ...
      >
        {creditCards.map((account) => {
          const itemData: ItemData = {
            id: createUniqueId('credit-card', 'ocorrences', account.id),
            originalId: account.id,
            type: 'incidents',
            category: 'ocorrences',
            name: account.name,
